...
const App: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const fetchPatientsAndDiags = async () => {
      try {
        const { data: patientListFromApi } = await axios.get<Patient[]>(`${apiBaseUrl}/patients`);
        dispatch(setPatientList(patientListFromApi));

        const { data: diagListFromApi } = await axios.get<Diagnosis[]>(`${apiBaseUrl}/diagnoses`);
        dispatch(setDiagnoses(diagListFromApi));
      } catch (e) {
        console.error(e);
      }
    };
    fetchPatientsAndDiags();
  }, [dispatch]);
